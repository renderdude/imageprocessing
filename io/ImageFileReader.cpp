...
ImageFileReader::readImage() {
	...
	if (_filename.find(".feat") == _filename.size() - 5) {
		...
		LOG_DEBUG(imagefilereaderlog) << "found simple image file, using own importer" << std::endl;
		...
		LOG_DEBUG(imagefilereaderlog) << "reading image of size " << width << "x" << height << std::endl;
		...
		*_image = _imageData;
		...
		LOG_DEBUG(imagefilereaderlog) << "Read data  (0,0): " << _imageData[0,0] << std::endl;
		LOG_DEBUG(imagefilereaderlog) << "Read image (0,0): " << (*_image)(0,0) << std::endl;
		return;
	}
